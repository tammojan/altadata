...
wait #iput
irsync -rsl %s/%s i:%s/%s > logs/transfer_%s_to_alta_verify.log 2>&1
...
""" % (dname,fname,fname,fname,fname,(' %s/' % fname).join(files),dname,fname,fname,fname,(' %s/' % fname).join(files),dname,fname,fname,fname,fname,fname))
out.flush()
